...
def pad_waveforms(w_1: torch.Tensor, w_2: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    l_w_1 = len(w_1)
    l_w_2 = len(w_2)
...
def normalize_tensor(tensor: torch.Tensor, reverse: bool = False, original_max: float = 0, original_min: float = 0) -> torch.Tensor:
    if not reverse:
