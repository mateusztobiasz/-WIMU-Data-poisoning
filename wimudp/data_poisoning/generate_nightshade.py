...
from wimudp.data_poisoning.utils import CONCEPT_C
...
def main():
    ...
    concept_c = input(
        "Enter value for CONCEPT_C (or press Enter to use default): "
    ).strip()
    concept_c = CONCEPT_C if concept_c == "" else concept_a

    concept_c_action_extractor = input(
        "Enter value for CONCEPT_C_ACTION (or press Enter to use default): "
    ).strip()
    concept_c_action_extractor = (
        None if concept_c_action_extractor == "" else concept_c_action_extractor
    )
    ...
    df_filtered.head(rows_number).to_csv(f"./data/audiocaps_{concept_c}.csv")
    print("Filtered DataFrame:")
    print(df_filtered)

    display_stage("Extracting samples")
    df_samples = get_samples(
        concept_c=concept_c or None,
        concept_c_action=concept_c_action_extractor or None,
